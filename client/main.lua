...
function OpenFurnitureMenu()
    ...
    local furnitureMenu = {
        ...
        { header = "7 - Flatten Rotation " .. objName, params = { type = "client", event = "housing:client:rotate", args = "flatten" } },
        { header = "8 - Sharp Rotation " .. objName, params = { type = "client", event = "housing:client:rotate", args = "sharp" } },
        { header = "9 - Collision: " .. colText, params = { type = "client", event = "housing:client:toggleCollision" } },
        { header = "💾 ยืนยันการบันทึกข้อมูล", params = { type = "client", event = "housing:client:saveFurniture" } },
        { header = "❌ ยกเลิกและลบทิ้ง", params = { type = "client", event = "housing:client:cancelFurniture" } }
    }
...
RegisterNetEvent('housing:client:rotate', function(type)
    ...
    elseif type == "flatten" then
        activeRot = vector3(0.0, 0.0, activeRot.z)
    end
    SetEntityRotation(currentObj, activeRot.x, activeRot.y, activeRot.z, 2, true)
    OpenFurnitureMenu()
end)
